            ...
            using (var bitmapBuilder = new BitmapBuilder(PluginImageSize.Width90))
            {
                ...
                switch (gaugeType)
                {
                    ...
                    case LibreHardwareMonitorGaugeType.DiskT6:
                        ...
                        bitmapBuilder.DrawText($"{levels[0]:N0}℃", x, ys[2], width, height, fontColor, fontSize);
                        break;
                    ...
                    case LibreHardwareMonitorGaugeType.CPUMonitor:
                        ...
                        levels[0] = this._lastMonLevels[gt];
                        ...
                        levels[1] = this._lastMonLevels[gt];
                        ...
                        levels[2] = this._lastMonLevels[gt];
                    ...
                    case LibreHardwareMonitorGaugeType.RAMMonitor:
                        ...
                        bitmapBuilder.DrawText($"[S] {this._lastMonLevels[(Int32)LibreHardwareMonitorGaugeType.RAM]:N1}G",         x, ys[0], width, height, this.GetColorByLevel(levels[0], maxLevels[0], accentColor), monFontSize);
                        bitmapBuilder.DrawText($"[V] {this._lastMonLevels[(Int32)LibreHardwareMonitorGaugeType.VrRAM]:N1}G",       x, ys[1], width, height, this.GetColorByLevel(levels[1], maxLevels[1], accentColor), monFontSize);
                        bitmapBuilder.DrawText($"[G] {this._lastMonLevels[(Int32)LibreHardwareMonitorGaugeType.VRAM] / 1024:N2}G", x, ys[2], width, height, this.GetColorByLevel(levels[1], maxLevels[2], accentColor), monFontSize);
                        break;
                    ...
                    case LibreHardwareMonitorGaugeType.DT1Monitor:
                        ...
                        for (var i = 0; i < 3; i++)
                        {
                            maxLevels[i]  = this._lastMaxLevels[(Int32)LibreHardwareMonitorGaugeType.DiskT1 + i];
                            levels[i]     = this._lastMonLevels[(Int32)LibreHardwareMonitorGaugeType.DiskT1 + i];
                            guageTypes[i] = (Int32)LibreHardwareMonitorGaugeType.DiskT1 + i;
                            if (levels[i] > 0)
                            {
                                //PluginLog.Info($"NVME{i + 1}: " + levels[i] + "℃," + x + "," + ys[i] + "," + sensor.Color + "," + titleFontSize + "," + monFontSize + "," + gaugeType + "," + imageIndex);
                    ...
                    case LibreHardwareMonitorGaugeType.DT2Monitor:
                        ...
                        for (var i = 0; i < 3; i++)
                        {
                            maxLevels[i]  = this._lastMaxLevels[(Int32)LibreHardwareMonitorGaugeType.DiskT4 + i];
                            levels[i]     = this._lastMonLevels[(Int32)LibreHardwareMonitorGaugeType.DiskT4 + i];
                            guageTypes[i] = (Int32)LibreHardwareMonitorGaugeType.DiskT4 + i;
                            if (levels[i] > 0)
                            {
                                //PluginLog.Info($"NVME{i + 4}: " + levels[i] + "℃," + x + "," + ys[i] + "," + sensor.Color + "," + titleFontSize + "," + monFontSize + "," + gaugeType + "," + imageIndex);
                    ...
                    case LibreHardwareMonitorGaugeType.DU1Monitor:
                        ...
                        for (var i = 0; i < 3; i++)
                        {
                            maxLevels[i]  = this._lastMaxLevels[(Int32)LibreHardwareMonitorGaugeType.DiskU1 + i];
                            levels[i]     = this._lastMonLevels[(Int32)LibreHardwareMonitorGaugeType.DiskU1 + i];
                    ...
                    case LibreHardwareMonitorGaugeType.DU2Monitor:
                        ...
                        for (var i = 0; i < 3; i++)
                        {
                            maxLevels[i]  = this._lastMaxLevels[(Int32)LibreHardwareMonitorGaugeType.DiskU4 + i];
                            levels[i]     = this._lastMonLevels[(Int32)LibreHardwareMonitorGaugeType.DiskU4 + i];
                ...
                if (gaugeType != LibreHardwareMonitorGaugeType.CPUMonitor
                    ...
                    && gaugeType != LibreHardwareMonitorGaugeType.DT2Monitor
                    && gaugeType != LibreHardwareMonitorGaugeType.DU1Monitor
                    && gaugeType != LibreHardwareMonitorGaugeType.DU2Monitor)
                {
                    //PluginLog.Info($"g{imageIndex}.png");
                    var imageBytes = PluginResources.ReadBinaryFile($"g{imageIndex}.png");
                    bitmapBuilder.DrawImage(imageBytes, x, ys[0] - 12);
                }
